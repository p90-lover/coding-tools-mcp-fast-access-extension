(() => {
  ...
  const buttons = (root) => [...(root?.querySelectorAll('button,[role="button"],[role="menuitem"]') || [])].filter(usable);
  ...
  function inspect() {
    ...
    const surfaceText = [alerts.map((el) => text(el)).join(' '), text(turn), text(assistant)].join(' ').slice(0, 4000);
    const blockedText = /too many requests|rate limit|usage limit|reached.{0,30}limit|sign in|log in|verify.{0,20}human|captcha|permission|approval|使用上限|已達上限|达到上限|驗證|验证|登入|登錄|權限|批准/i;
    ...
    const thinkingFailed = /thinking failed|thought failed|reasoning failed|思考失敗|思考失败|思考出错|思考出錯/i.test(surfaceText);
    const mcpDisabled = /mcp.{0,24}(disabled|forbidden)|tool.{0,30}disabled|developer mode.{0,24}(off|disabled)|FORBIDDEN|工具.{0,12}(已停用|已禁用|被停用)|MCP.{0,16}(停用|禁用|禁止|不可用)/i.test(surfaceText)
      || Boolean(globalThis.__ctmHudMcpDisabled);
    const choices = buttons(turn);
    const retry = thinkingFailed ? null : choices.find((el) => /^(try again|retry|regenerate(?: response)?|重新嘗試|再試一次|重試|重试|重新生成|重新產生)$/i.test(label(el)));
    const more = choices.find((el) => /^(continue generating|繼續產生|繼續生成|继续生成)$/i.test(label(el)));
    const branch = [...buttons(turn), ...buttons(main), ...buttons(document)].find((el) =>
      /^(branch(?: from here)?|branch chat|建立分支|建立對話分支|分支)$/i.test(label(el)));
    ...
    const manualStop = localPause || /you stopped this response|response stopped|你已停止|您已停止|已停止回應|已停止生成/i.test(surfaceText + ' ' + lastText);
    const blocked = !main || !editor || blockedText.test(surfaceText)
    ...
    return {
      sample: {
        hasUser: !!user, userKey, userTextHash: P.hash(text(user)),
        fingerprint: P.hash(`${userKey}:${assistant?.getAttribute('data-message-id') || ''}:${text(assistant)}:${surfaceText}:${!!retry}:${!!more}:${thinkingFailed}:${mcpDisabled}`),
        busy, blocked: blocked && !mcpDisabled, manualStop,
        draft: !!text(editor) || attachmentPresent(editor), editing: Date.now() - lastInput < 10000,
        error: !thinkingFailed && failureText.test(surfaceText), retry: !!retry, continueButton: !!more,
        finished: !!assistant && !needsInput && !thinkingFailed && choices.some((el) => el.matches('[data-testid="copy-turn-action-button"],[data-testid="good-response-turn-action-button"],[data-testid="bad-response-turn-action-button"]')),
        thinkingFailed, mcpDisabled, branchButton: !!branch,
      },
      retry, more, branch, editor,
    };
  }
  ...
  function rpc(data) {
    return new Promise((resolve, reject) => {
      try {
        ...
        port.postMessage({ ...data, id, url: location.href, chatId: P.conversationKey(location.href), generation: config.generation });
  ...
  function accept(next) {
    ...
    config = next;
    window.dispatchEvent(new CustomEvent('ctm-recovery-view', { detail: next }));
  }

  async function hello() {
    lastUrl = location.href;
    if (!P.conversationKey(lastUrl)) { config.enabled = false; window.dispatchEvent(new CustomEvent('ctm-recovery-view', { detail: config })); return; }
  ...
  function unchanged(plan, state) {
    return config.enabled && !localPause && plan.generation === config.generation && plan.key === P.conversationKey(location.href)
      ...
      && (!state.sample.draft || plan.kind === 'send_branch') && !state.sample.editing && !state.sample.manualStop;
  }
  ...
  async function execute(plan) {
    ...
    if (plan.kind === 'send_branch' && fresh.sample.mcpDisabled && usable(fresh.branch)) {
      const previous = P.conversationKey(location.href);
      fresh.branch.click();
      for (let i = 0; i < 40; i++) {
        await new Promise((resolve) => setTimeout(resolve, 250));
        const next = P.conversationKey(location.href);
        if (next && next !== previous) {
          accept(await rpc({ type: 'arm_followup', url: location.href, chatId: next }));
          return;
        }
      }
      return;
    }
    if (plan.kind !== 'send_continue' || !(fresh.sample.finished || fresh.sample.thinkingFailed) || !config.options?.autoContinue) return;
